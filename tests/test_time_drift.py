...
def client(monkeypatch):
    instance = BinanceClient("key", "secret", base_url="https://demo-fapi.binance.com", testnet=True)
...
def test_time_drift_warning_logs_event(monkeypatch, client):
    ...
    monkeypatch.setattr(infra_logging, "log_event", lambda event, **fields: events.append((event, fields)))
