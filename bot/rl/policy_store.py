...
class RLPolicyStore:
        ...
        *,
        ...
        logger.info(
            ...
            extra={
                ...
                "run_id": run_metadata.run_id,
                "param_deviation": param_deviation,
                "val_reward_mean": metrics.get("val_reward_mean", float("nan")),
                "baseline_params_hash": run_metadata.baseline_params_hash,
            },
        )
        return version
    ...
    def list_policies(self, policy_name: str) -> List[RLPolicyVersion]:
        ...
        for candidate in policy_dir.iterdir():
            ...
            try:
                ...
                logger.warning("Skipping invalid RL policy metadata", extra={"metadata_path": str(metadata_path)})
                continue
    ...
    def _load_payload(
        ...
        try:
            ...
            logger.warning(
                "Failed to parse RL policy payload",
                extra={"policy_name": policy_name, "version_id": version.version_id},
            )
            return None
